...
#:package Microsoft.CognitiveServices.Speech@1.46.0
#:package System.Net.Http.Json@9.0.9
...
/// </summary>
SpeechConfig CreateSpeechConfig(string language)
{
...
/// マイクからリアルタイム話者分離
/// </summary>
async Task TranscribeFromMicrophoneAsync(string language)
{
...
/// WAVファイルから話者分離
/// </summary>
async Task TranscribeFromFileAsync(string language)
{
    ...
    await TranscribeFromFileWithPathAsync(language, filePath);
}

/// <summary>
/// WAVファイルから話者分離（ファイルパス指定版）
/// </summary>
async Task TranscribeFromFileWithPathAsync(string language, string filePath)
{
...
/// 転写結果をファイルに保存
/// </summary>
async Task SaveTranscriptionResultsAsync(string originalFilePath, List<(string SpeakerId, string Text, TimeSpan StartTime, TimeSpan EndTime)> results, string language)
{
    try
    {
...
/// WAVファイルから高速バッチ話者分離
/// </summary>
async Task BatchTranscribeFromFileAsync(string language)
{
    Console.Write("WAVファイルのパスを入力してください: ");
    var filePath = Console.ReadLine();
    ...
    await BatchTranscribeFromFileWithPathAsync(language, filePath);
}

/// <summary>
/// WAVファイルから高速バッチ話者分離（ファイルパス指定版）
/// </summary>
async Task BatchTranscribeFromFileWithPathAsync(string language, string filePath)
{
...
/// コマンドライン引数の処理
/// </summary>
async Task ProcessCommandLineArgs(string[] args)
{
    // ヘルプ表示
...
/// 言語コードの解析
/// </summary>
string ParseLanguage(string input)
{
    // 短縮形の言語名対応
...
// Batch API用のデータクラス
public record BatchTranscriptionRequest
{
...
public record BatchTranscriptionProperties
{
...
public record DiarizationSettings
{
...
public record BatchTranscriptionLinks
{
    [JsonPropertyName("files")]
    public string Files { get; set; } = "";
}

public record BatchTranscriptionFiles
{
    [JsonPropertyName("values")]
    public BatchTranscriptionFile[] Values { get; set; } = Array.Empty<BatchTranscriptionFile>();
}

public record BatchTranscriptionFile
{
